...
describe('CmsController', () => {
  ...
  it('approveReview는 APPROVED 상태로 위임해야 한다', async () => {
    ...
    expect(service.reviewDecision).toHaveBeenCalledWith('req-1', 'operator-1', {
  ...
  it('rejectReview는 REJECTED 상태와 사유를 전달해야 한다', async () => {
    ...
    expect(service.reviewDecision).toHaveBeenCalledWith('req-2', 'operator-1', {
  ...
  it('rollback은 versionNo를 숫자로 전달해야 한다', async () => {
    ...
    expect(service.rollbackLesson).toHaveBeenCalledWith(
      'lesson-1',
      2,
      'operator-1',
    );
