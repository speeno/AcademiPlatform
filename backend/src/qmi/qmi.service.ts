...
import { QmiDocumentService, QmiRetrievedDoc } from './document.service';
import { QmiOpenAiService } from './openai.service';

export type QmiReplySource = 'intent' | 'knowledge' | 'fallback';
...
export interface QmiChatResult {
  reply: string;
  /** intent: 인사/감사 등 / knowledge: 문서 근거 답변 / fallback: 데이터 폴백 안내 */
  ...
  /** 답변 근거로 사용된 문서 출처 */
  sources?: QmiSource[];
  /** 검색 경로(vector|lexical|memory) — 디버깅/관찰용 */
  retrieval?: string;
}
...
export class QmiService {
  ...
  async chat(rawMessage: string): Promise<QmiChatResult> {
    ...
    const intent = this.detectIntent(compactMsg);
    if (intent && compactMsg.length <= 12) return intent;

    // 2) 검색(retrieve) → grounded 생성
    const retrieved = await this.documents.search(message, 4);
    ...
    // 2-1) intent 가 있었으나 길어서 건너뛴 경우
    if (intent) return intent;

    // 3) 데이터 폴백 — 매칭 실패 시 안내 + 추천
    return {
      reply:
