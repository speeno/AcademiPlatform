  ...
  return (
    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 space-y-1.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-muted-foreground">
        <span>입금은행: <span className="font-medium text-foreground">{account.bank}</span></span>
        <span>예금주: <span className="font-medium text-foreground">{account.holder}</span></span>
        <span className="sm:col-span-2">
          입금계좌: <span className="font-semibold text-foreground tracking-wide">{account.account}</span>
        </span>
      </div>
    </div>
  );
