...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    const fetch_ = async () => {
      ...
      try {
        const res = await fetch(`${API}/exam/my/applications`, {
  ...
  const handleCancel = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const res = await fetch(`${API}/exam/my/applications/${id}/cancel`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token ?? ''}` },
      });
              ...
              <div key={app.id} className="bg-white rounded-xl border p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h3 className="font-bold text-gray-900">{app.examSession.qualificationName}</h3>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-gray-500">
                      <span>시험일: {new Date(app.examSession.examAt).toLocaleDateString('ko-KR')}</span>
                      {app.examSession.place && <span>장소: {app.examSession.place}</span>}
                      <span>응시료: {app.examSession.fee.toLocaleString()}원</span>
