    ...
    <div>
      <section className="bg-hero-gradient py-14 border-b border-border">
        <PageShell size="content" flush>
          <h1 className="text-heading mb-2 text-brand-blue">개인정보처리방침</h1>
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-6 bg-card rounded-xl border border-border p-4">
              ...
              <ul className="space-y-1">
                {sections.map((s) => (
                  <li key={s.title}>
                    <a href={`#${s.title}`} className="text-xs block py-0.5 text-brand-blue hover:underline">
                      {s.title}
                    </a>
                  </li>
                ))}
