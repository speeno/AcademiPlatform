...
export default function AdminCoursesPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const target = selectedId
        ? nextCourses.find((course: CourseItem) => course.id === selectedId) ?? nextCourses[0]
        : nextCourses[0];
  ...
  const removeModule = async (moduleId: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('모듈 삭제에 실패했습니다.');
      await loadDetail(selectedId);
  ...
  const removeLesson = async (moduleId: string, lessonId: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('레슨 삭제에 실패했습니다.');
      if (selectedId) await loadDetail(selectedId);
          ...
          <div className="bg-white rounded-xl border p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                />
                <BrandButton size="sm" onClick={addModule} disabled={!selectedId}>
            ...
            {modules.length === 0 ? (
              <p className="text-sm text-gray-500">모듈이 없습니다.</p>
            ) : (
              modules.map((module) => (
                <div key={module.id} className="border rounded-lg p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <BrandButton size="sm" variant="secondary" onClick={() => saveModule(module.id, { title: module.title, sortOrder: module.sortOrder })}>
                      저장
                    </BrandButton>
                    <BrandButton size="sm" variant="outline" onClick={() => removeModule(module.id)}>
                    ...
                    {module.lessons.map((lesson) => (
                      <div key={lesson.id} className="grid md:grid-cols-[1fr_120px_90px_70px_80px] gap-2 items-center">
                        ...
                        <div className="flex gap-1">
                          <BrandButton size="sm" variant="secondary" onClick={() => saveLesson(module.id, lesson.id, {
                            title: lesson.title,
                            lessonType: lesson.lessonType,
                            sortOrder: lesson.sortOrder,
                            isPreview: lesson.isPreview,
                          })}>저장</BrandButton>
                          <BrandButton size="sm" variant="outline" onClick={() => removeLesson(module.id, lesson.id)}>
                            <Trash2 className="w-4 h-4" />
                          </BrandButton>
                        </div>
                      </div>
                  ...
                  <div className="grid md:grid-cols-[1fr_140px_100px] gap-2">
                    ...
                    <BrandButton size="sm" onClick={() => addLesson(module.id)}>
